...
contract BVC {
    ...
    function setCandidate(uint _placeID) public {
    ...
    function getCandidate() public view returns(uint) { //return으로 후보자 ID
    ...
    function getPlaceLength() public view returns(uint){
        return placeList.length;
    }

    function getPlaceId(uint index) public view returns(uint, bool){
    ...
    function getCandidateId(uint index) public view returns(uint, uint) {
    ...
    function setVote(uint _placeID, uint _candidateID, uint _phone) public {
         voterList.length += 1;
    ...
    function getCheckVoted(uint _placeID, string memory _phone) public view returns(bool) {
        for (uint i = 0; i < voterList.length; i++) {
            if (keccak256(abi.encodePacked(voterList[i].phone)) == keccak256(abi.encodePacked(_phone)) && voterList[i].votedPlace == _placeID) {
                return (true);
            }
        }
        return (false);
    ...
    // 투표를 시작하도록 하는 함수
    function setVoteStart(uint _placeID) public{
        setStarted(_placeID, true);
    }
    // 투표를 중지하는 함수
    function setVoteEnd(uint _placeID) public {
        setStarted(_placeID, false);
    }

    function setStarted(uint _placeID, bool started) public{
    ...
    function getCounting(uint _placeID, uint _index) public view returns(uint, uint, uint) {
        for (uint i = 0; i < placeList.length; i++) {
